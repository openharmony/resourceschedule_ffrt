...
int ffrt_epoll_ctl(ffrt_qos_t qos, int op, int fd, uint32_t events, void* data, ffrt_poller_cb cb)
{
    ...
    if (op == EPOLL_CTL_DEL) {
        return ffrt::FFRTFacade::GetIOPoller().DelFdEvent(fd);
    } else if (op == EPOLL_CTL_ADD || op == EPOLL_CTL_MOD) {
        return ffrt::FFRTFacade::GetIOPoller().AddFdEvent(op, events, fd, data, cb);
    } else {
...
int ffrt_epoll_wait(ffrt_qos_t qos, struct epoll_event* events, int max_events, int timeout)
{
    ...
    return ffrt::FFRTFacade::GetIOPoller().WaitFdEvent(events, max_events, timeout);
}
...
void ffrt_poller_wakeup(ffrt_qos_t qos)
{
    ...
    ffrt::FFRTFacade::GetIOPoller().WakeUp();
}
...
uint8_t ffrt_epoll_get_count(ffrt_qos_t qos)
{
    ...
    return ffrt::FFRTFacade::GetIOPoller().GetPollCount() % UINT8_MAX;
}
...
uint64_t ffrt_epoll_get_wait_time(void* taskHandle)
{
    ...
    return ffrt::FFRTFacade::GetIOPoller().GetTaskWaitTime(task);
}