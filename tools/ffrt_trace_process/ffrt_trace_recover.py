...
def make_coyield_fake_log(mark, pid, label, gid, tid, tname, prio):
    ...
    timestamp = extract_timestamp_str(mark)
    cpu_id = extract_cpu_id_str(mark)
    fake_log = "  %s-%d00%d    (%7d) [%s] ....   %s: sched_switch: prev_comm=%s prev_pid=%d00%d prev_prio=%d prev_state=S ==> next_comm=%s next_pid=%d next_prio=%d\n" % (
        label, pid, gid, pid, cpu_id, timestamp, label, pid, gid, prio, tname, tid, prio)

    if "|B|" in mark:
        fake_log = "  %s-%d00%d    (%7d) [%s] ....   %s: tracing_mark_write: E|%d\n" % \
...
def replace_sched_switch_log(fake_log, mark, pid, label, gid, tid):
    ...
    if "prev_pid=%d" % tid in mark:
        ...
        fake_log = "  %s-%d00%d " % (label, pid, gid) + fake_log[index:]
        fake_log = fake_log[:fake_log.index("prev_comm=")] + "prev_comm=%s " % label + \
                   fake_log[fake_log.index("prev_pid="):]
        fake_log = fake_log[:fake_log.index("prev_pid=")] + "prev_pid=%d00%d " % (pid, gid) + \
        ...
        fake_log = fake_log[:fake_log.index("next_pid=")] + "next_pid=%d00%d " % (pid, gid) + \
...
def replace_sched_wake_log(fake_log, label, pid, gid):
    ...
    fake_log = fake_log[:fake_log.index("pid=")] + "pid=%d00%d " % (pid, gid) + fake_log[fake_log.index("prio="):]
...
def replace_sched_block_log(fake_log, pid, gid):
    ...
    """
    fake_log = fake_log[:fake_log.index("pid=")] + "pid=%d00%d " % (pid, gid) + fake_log[fake_log.index("iowait="):]
        ...
        for tid, info in ffrt_pids[pid].items():
            ...
            for lineno in linenos:
                ...
                if task_running is not None:
                    ...
                    if re.search(r" F\|(\d+)\|Co\|(\d+)", mark) is not None:
                        logs[lineno] = "\n"
    ...
    for lineno in range(len(logs)):
        ...
        if "FFWK|" in log:
            ...
            if pid in task_labels.keys() and gid in task_labels[pid].keys():
                if task_labels[pid][gid]["state"] == "block":
                    timestamp = extract_timestamp_str(log)
                    cpu_id = extract_cpu_id_str(log)
                    ready_begin_log = "  <...>-%d00%d    (%7d) [%s] ....   %s: tracing_mark_write: B|%d|FFREADY\n" % (
        ...
        if "FFRT::[" in log:
            ...
            if pid in task_labels.keys() and gid in task_labels[pid].keys():
                if task_labels[pid][gid]["state"] == "ready":
                    ...
                    ready_end_log = "  <...>-%d00%d    (%7d) [%s] ....   %s: tracing_mark_write: E|%d\n" % (
                        pid, gid, pid, cpu_id, timestamp, pid)
...
def main():
    ...
    with open(args.file, 'r', encoding="gb18030", errors="ignore") as infile:
