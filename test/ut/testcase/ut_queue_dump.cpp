...
static const int THREAD_SIZE = 10 * 1024 * 1024;
...
static void QueueDumpMatchFunc(void*(*ThreadFunc)(void*), void *buf)
{
    pthread_t thread;
    pthread_attr_t queue_attr;
    pthread_attr_init(&queue_attr);
    pthread_attr_setstacksize(&queue_attr, THREAD_SIZE);
    pthread_create(&thread, &queue_attr, ThreadFunc, buf);
...
static void QueueDumpTask1Test(ffrt_queue_t& queue_handle, char* buf)
{
    ...
    // 4.无剩余未执行的任务
    QueueDumpMatchFunc(Task1MatchFunc, buf);

    ffrt_task_handle_destroy(t);
    ffrt_task_attr_destroy(&task_attr);
}

void *Task2MatchFunc(void *buf)
{
    std::string str((char *)buf);
...
static void QueueDumpTask2Test(ffrt_queue_t& queue_handle, char* buf)
{
    ...
    // 3.有32条历史执行的任务
    // 4.无剩余未执行的任务
    QueueDumpMatchFunc(Task2MatchFunc, buf);

    ffrt_task_handle_destroy(t);
    ffrt_task_attr_destroy(&task_attr);
}

void *Task3MatchFunc(void *buf)
{
    std::string str((char *)buf);
...
static void QueueDumpTask3Test(ffrt_queue_t& queue_handle, char* buf)
{
    ...
    // 3.有32条历史执行的任务
    // 4.无剩余未执行的任务
    QueueDumpMatchFunc(Task3MatchFunc, buf);
...
void *PriorityMatchFunc(void *buf)
{
    std::string str((char *)buf);
...
static void QueueDumpPriorityTest(ffrt_queue_t& queue_handle, char* buf)
{
    ...
    // 3.有32条历史执行的任务
    // 4.5种优先级各有10条任务
    QueueDumpMatchFunc(PriorityMatchFunc, buf);
